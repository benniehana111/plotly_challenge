...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sample_id = sample_ata.filter(sampleObj => sampleObj.id == sample);
    ...
    top10_sample_values = top10_otu_ids.map((id) =>
                firstSample.sample_values[firstSample.otu_ids.indexOf(id)]);    
    
    top10_otu_labels = top10_otu_ids.map((id) =>
                firstSample.otu_labels[firstSample.otu_ids.indexOf(id)]);                   
